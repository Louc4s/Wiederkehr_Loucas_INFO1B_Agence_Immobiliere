...
"""
    ...
    Paramètres : id_type_sel = 0 >> tous les films.
                 id_type_sel = "n" affiche le film dont l'id est "n"
    ...
    if request.method == "GET":
        try:
            with DBconnection() as mc_afficher:
                strsql_genres_films_afficher_data = """SELECT id_biens, fk_client, nom_bien, description_bien, nb_piece, prix,
                                                            GROUP_CONCAT(intitule_type) as GenresFilms FROM t_type_bien
                                                            RIGHT JOIN t_biens ON t_biens.id_biens = t_type_bien.fk_bien
                                                            LEFT JOIN t_type ON t_type.id_type = t_type_bien.fk_type
                                                            GROUP BY id_biens"""
                if id_film_sel == 0:
                    ...
                    strsql_genres_films_afficher_data += """ HAVING id_biens = %(value_id_film_selected)s"""
                ...
                if not data_genres_films_afficher and id_film_sel == 0:
                    ...
                    flash(f"Le type {id_film_sel} demandé n'existe pas !!", "warning")
                else:
                    flash(f"Données types et biens affichés !!", "success")
...
"""
    ...
    1) Tous les genres contenus dans la "t_type".
...
@app.route("/edit_type_film_selected", methods=['GET', 'POST'])
def edit_type_film_selected():
    if request.method == "GET":
        try:
            with DBconnection() as mc_afficher:
                strsql_genres_afficher = """SELECT id_type, intitule_type FROM t_type ORDER BY id_type ASC"""
                mc_afficher.execute(strsql_genres_afficher)
            data_genres_all = mc_afficher.fetchall()
            print("dans edit_type_film_selected ---> data_genres_all", data_genres_all)
            ...
            # href="{{ url_for('edit_type_film_selected', id_film_genres_edit_html=row.id_film) }}"
            ...
            lst_data_film_selected = [item['id_biens'] for item in data_genre_film_selected]
            ...
            # Extrait les valeurs contenues dans la table "t_types", colonne "intitule_type"
            # Le composant javascript "tagify" pour afficher les tags n'a pas besoin de l'id_type
            lst_data_genres_films_non_attribues = [item['intitule_type'] for item in data_genres_films_non_attribues]
            print("lst_all_genres gf_edit_type_film_selected ", lst_data_genres_films_non_attribues,
                  type(lst_data_genres_films_non_attribues))

        except Exception as Exception_edit_type_film_selected:
            raise ExceptionEditGenreFilmSelected(f"fichier : {Path(__file__).name}  ;  "
                                                 f"{edit_type_film_selected.__name__} ; "
                                                 f"{Exception_edit_type_film_selected}")
...
def update_genre_film_selected():
    if request.method == "POST":
        try:
            ...
            new_lst_int_type_film_old = list(map(int, new_lst_str_genres_films))
            print("new_lst_type_film ", new_lst_int_type_film_old, "type new_lst_type_film ",
                  type(new_lst_int_type_film_old))

            # Pour apprécier la facilité de la vie en Python... "les ensembles en Python"
            # https://fr.wikibooks.org/wiki/Programmation_Python/Ensembles
            # OM 2021.05.02 Une liste de "id_type" qui doivent être effacés de la table intermédiaire "t_type_film".
            lst_diff_genres_delete_b = list(set(old_lst_data_genres_films_attribues) -
                                            set(new_lst_int_type_film_old))
            print("lst_diff_genres_delete_b ", lst_diff_genres_delete_b)

            # Une liste de "id_type" qui doivent être ajoutés à la "t_type_film"
            lst_diff_genres_insert_a = list(
                set(new_lst_int_type_film_old) - set(old_lst_data_genres_films_attribues))
            print("lst_diff_genres_insert_a ", lst_diff_genres_insert_a)

            # SQL pour insérer une nouvelle association entre
            # "fk_film"/"id_film" et "fk_genre"/"id_type" dans la "t_type_film"
            strsql_insert_type_film = """INSERT INTO t_type_bien (id_type_bien, fk_type, fk_bien)
                                                    VALUES (NULL, %(value_fk_genre)s, %(value_fk_film)s)"""

            # SQL pour effacer une (des) association(s) existantes entre "id_film" et "id_type" dans la "t_type_film"
            strsql_delete_genre_film = """DELETE FROM t_type_bien WHERE fk_type = %(value_fk_genre)s AND fk_bien = %(value_fk_film)s"""

            with DBconnection() as mconn_bd:
                # Pour le film sélectionné, parcourir la liste des genres à INSÉRER dans la "t_type_film".
                ...
                for id_type_ins in lst_diff_genres_insert_a:
                    ...
                    valeurs_film_sel_genre_sel_dictionnaire = {"value_fk_film": id_film_selected,
                                                               "value_fk_genre": id_type_ins}

                    mconn_bd.execute(strsql_insert_type_film, valeurs_film_sel_genre_sel_dictionnaire)

                # Pour le film sélectionné, parcourir la liste des genres à EFFACER dans la "t_type_film".
                # Si la liste est vide, la boucle n'est pas parcourue.
                for id_type_del in lst_diff_genres_delete_b:
                    # Constitution d'un dictionnaire pour associer l'id du film sélectionné avec un nom de variable
                    # et "id_type_del" (l'id du genre dans la liste) associé à une variable.
                    valeurs_film_sel_genre_sel_dictionnaire = {"value_fk_film": id_film_selected,
                                                               "value_fk_genre": id_type_del}
    ...
    # Après cette mise à jour de la table intermédiaire "t_type_film",
...
def genres_films_afficher_data(valeur_id_film_selected_dict):
    ...
    try:
        ...
        strsql_film_selected = """SELECT id_biens, fk_client, nom_bien, description_bien, nb_piece, prix, GROUP_CONCAT(id_type) as GenresFilms FROM t_type_bien
                                        INNER JOIN t_biens ON t_biens.id_biens = t_type_bien.fk_bien
                                        INNER JOIN t_type ON t_type.id_type = t_type_bien.fk_type
                                        WHERE id_biens = %(value_id_film_selected)s"""

        strsql_genres_films_non_attribues = """SELECT id_type, intitule_type FROM t_type WHERE id_type not in(SELECT id_type as idGenresFilms FROM t_type_bien
                                                    INNER JOIN t_biens ON t_biens.id_biens = t_type_bien.fk_bien
                                                    INNER JOIN t_type ON t_type.id_type = t_type_bien.fk_type
                                                    WHERE id_biens = %(value_id_film_selected)s)"""

        strsql_genres_films_attribues = """SELECT id_biens, id_type, intitule_type FROM t_type_bien
                                            INNER JOIN t_biens ON t_biens.id_biens = t_type_bien.fk_bien
                                            INNER JOIN t_type ON t_type.id_type = t_type_bien.fk_type
                                            WHERE id_biens = %(value_id_film_selected)s"""
